...
SILENCE_DELAY = 1.0  # seconds of silence before considering speech ended
...
def _get_vosk_model():
    ...
    if _vosk_model is None:
        if not os.path.exists(MODEL_PATH):
            raise FileNotFoundError(f"Vosk model not found at {MODEL_PATH}. Please download models first.")
...
def test_vosk_recognition():
    ...
    try:
        ...
        print(f"📊 Audio settings: Sample rate={SAMPLE_RATE}Hz, Chunk size={CHUNK_SIZE}")
    ...
    try:
        ...
        while (time.time() - start_time) < duration:
            try:
                ...
                print(f"Raw audio: min={raw_min:6d} max={raw_max:6d} rms={raw_rms:6d}", end='\r', flush=True)
                time.sleep(0.1)
    ...
    try:
        ...
        while (time.time() - start_time) < duration:
            try:
                ...
                print(f"Max amplitude: {chunk_max:5d} (overall max: {max_amplitude})", end='\r', flush=True)
                time.sleep(0.1)
            except Exception as e:
